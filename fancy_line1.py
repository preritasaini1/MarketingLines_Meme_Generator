...
def generate_meme(image, top_text, bottom_text, font_size, text_color="white"):
    ...
    top_lines = split_text_to_fit(top_text, font, meme_image.width - 20, draw)
    bottom_lines = split_text_to_fit(bottom_text, font, meme_image.width - 20, draw)
...
with tab2:
    ...
    meme_topic = st.selectbox(
        "Select a Meme Topic",
        ["Social Welfare", "Study", "Laugh", "Knowledge","Others"]
    )
    ...
    def generate_meme_line(topic):
        ...
        The text should be concise and fit within the image dimensions.
        For the selected topic: {topic}, create a single short and catchy line in Hinglish (a mix of Hindi and English) 
        that is fun, relatable, and perfect for a meme which makes sense , written hindi in english only.

        """
        response = model.generate_content(prompt)
        return response.text.strip().split(",")  # Get a single line of text
    if meme_topic == "Others":
    ...
    if uploaded_image:
        # Generate the text dynamically based on the topic
            
        st.info(f"Creating a witty line for the topic: {meme_topic}...")
        if meme_topic == "Others":
            meme_line = generate_meme_line(custom_topic)
        else:
            meme_line = generate_meme_line(meme_topic)

        # Provide user-editable fields for top and bottom text
        top_text = st.text_input("Top Text", meme_line[0] or "TOP TEXT")
        bottom_text = st.text_input("Bottom Text", meme_line[1] or "BOTTOM TEXT")

        # Font size slider
        font_size = st.slider("Font Size", 5, 20, 13)
        ...
        if st.button("Generate Meme"):
            ...
            st.info("Click 'Generate Meme' to create your customized meme!")
    else:
